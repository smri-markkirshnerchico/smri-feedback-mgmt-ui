...
export const formatPatterns = {
  dateTime: 'MMM DD YYYY h:mm a', // Apr 17 2022 12:00 am
  ...
  iso: {
    dateTime: 'YYYY-MM-DDTHH:mm:ss' // 2022-04-17T00:00:00
  }
};
...
/**
 * @output 2022-04-17T00:00:00
 */
export function fDateTimeISO(date: DatePickerFormat): string {
  if (!isValidDate(date)) {
    return 'Invalid date';
  }

  return dayjs(date).format(formatPatterns.iso.dateTime);
}
...
export function fDateOptional(input: DatePickerFormat, template?: string): string {
  if (!isValidDate(input)) {
    return input?.toString() ?? '';
  }

  return dayjs(input).format(template ?? formatPatterns.date);
}
